...
public class EntityOreFX extends EntityFX
{
    ...
    public EntityOreFX(World var1, double var2, double var4, double var6, float var8, float var9, float var10, int var11)
    {
        ...
        this.noClip = true;
        this.setParticleTextureIndex(Math.random() > 0.5D ? 2 + 16 * var11 : 16 * var11);
        this.decending = this.getParticleTextureIndex() % 16 > 0;
        this.id = var11;
        this.rotation = (int)(Math.random() % 90.0D);
    }
    ...
    public void renderParticle(Tessellator var1, float var2, float var3, float var4, float var5, float var6, float var7)
    {
        ...
        GL11.glBindTexture(GL11.GL_TEXTURE_2D, ModLoader.getMinecraftInstance().renderEngine.getTexture("/shadow/EnderParticles.png"));
    ...
    public void onUpdate()
    {
        ...
        {
            this.setParticleTextureIndex(1 + 16 * this.id);
        }

        if (this.particleAge == this.particleMaxAge / 3 + this.particleMaxAge / 3)
        {
            if (this.decending)
            {
                this.setParticleTextureIndex(16 * this.id);
            }
            else
            {
                this.setParticleTextureIndex(2 + 16 * this.id);
            }
        }
